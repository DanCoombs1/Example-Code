...
# -*- coding: utf-8 -*-
"""
Created on Thu Mar 21 22:05:38 2024
...
vac_day = 210  #how many days the vaccine took to be released
...
def vaccination():
    """
    function that will give a person a vaccine given they meet certain criteria 
    - the vaccine must be available
    - the person must not be vaccinated for the first vaccine
    - the person has to have waited the correct time between doses for the second vaccine to be given
    - the person must not be infected
    - the daily limit of vaccines must not have been reached
    ...
    for x in range(col):
        for y in range(row):
            if vaccine_grid[x][y] == 0 and vac_going_ahead == True and days > vac_day and infection_grid[x][y] == 0: 
